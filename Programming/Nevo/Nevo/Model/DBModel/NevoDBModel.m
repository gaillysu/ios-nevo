...
+ (BOOL)createTable
{
    FMDatabase *db = [FMDatabase databaseWithPath:[NevoDBHelper dbPath]];
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
    }
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    for (NevoDBModel *model in array) {
        if (![model isKindOfClass:[NevoDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [nevoDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            NevoDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
